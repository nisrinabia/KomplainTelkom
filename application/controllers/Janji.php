...
class Janji extends CI_Controller {
    ...
    public function excel($mode)
    {
        ...
        if($mode == "all")
        {
            ...
            $this->db->select("k.NO_POTS, (CASE WHEN k.NO_INTERNET IS NULL THEN '-' ELSE k.NO_INTERNET END), (CASE WHEN k.NAMA_PELAPOR IS NULL THEN '-' ELSE k.NAMA_PELAPOR END), (CASE WHEN k.ALAMAT_PELAPOR IS NULL THEN '-' ELSE k.ALAMAT_PELAPOR END), (CASE WHEN k.PIC_PELAPOR IS NULL THEN '-' ELSE k.PIC_PELAPOR END), k.NAMA_MEDIA, l.NAMA_LAYANAN, j.JENIS, (CASE WHEN (k.TGL_KOMPLAIN = '0000-00-00 00:00:00' OR k.TGL_KOMPLAIN IS NULL) THEN '-' ELSE k.TGL_KOMPLAIN END) AS TANGGAL_KOMPLAIN, (CASE WHEN (k.TGL_CLOSE = '0000-00-00' OR k.TGL_CLOSE IS NULL) THEN '-' ELSE k.TGL_CLOSE END) AS TGL_CLOSE, (CASE WHEN k.DEADLINE = '0000-00-00 00:00:00' THEN '-' ELSE k.DEADLINE END) AS DEADLINE, (CASE WHEN k.KELUHAN IS NULL THEN '-' ELSE k.KELUHAN END), (CASE WHEN k.SOLUSI IS NULL THEN '-' ELSE k.SOLUSI END), (CASE WHEN k.KETERANGAN IS NULL THEN '-' ELSE k.KETERANGAN END), (CASE WHEN k.STATUS_JANJI = '0' THEN 'Belum ditangani' ELSE 'Telah ditangani' END), TIME_FORMAT(TIMEDIFF((k.DEADLINE),NOW()), '%H') as HOUR"); 
